...
static int dvc_fill_sd_bin_data_max_llr_view(const ldpc_packet *pckt, int *bin_data, int bin_elems)
{
    ...
    // HM_N export view:
    //   [User Data][User Padding=max][VP_A][PMSK=max][VP_B]
    //   0         info_len          hm_k              hm_k+Z hm_n
    ...
    if ((pckt->h_matrix.extra_bits_of_parity > 0) && (pckt->cir_sz > 0)) {
        int vp_a_bits = pckt->h_matrix.extra_bits_of_parity;
        if (vp_a_bits > pckt->cir_sz)
            vp_a_bits = pckt->cir_sz;
        if (vp_a_bits > parity_bits_in_det)
            vp_a_bits = parity_bits_in_det;

        for (int i = 0; i < vp_a_bits; i++) {
        ...
        const int vp_b_bits = parity_bits_in_det - vp_a_bits;
        for (int i = 0; i < vp_b_bits; i++) {
            const int src = pckt->info_len + vp_a_bits + i;
            const int dst = pckt->hm_k + pckt->cir_sz + i;
            if ((src >= 0) && (src < pckt->blk_len) && (dst >= 0) && (dst < pckt->hm_n))
                bin_data[dst] = (int)(unsigned char)pckt->det_blk[src];
        }
    } else {
...
static int dvc_fill_sd_bin_data_tail_mask_view(const ldpc_packet *pckt, int *bin_data, int bin_elems)
{
    if (!pckt || !bin_data || (bin_elems <= 0))
        return 0;
    ...
    // Keep bin_data in the BLK stream view regardless of rx_data_sv capacity.
    // This avoids silently switching to an HM_N view when DV allocates a larger
    // comparison buffer.
    for (int i = 0; i < pckt->blk_len && i < bin_elems; i++)
        bin_data[i] = (int)(unsigned char)pckt->det_blk[i];
    return 0;
...
static int dvc_fill_actual_bin_data_hm_view(const ldpc_packet *pckt,
                                            const int *src_bins,
                                            int src_elems,
                                            int *hm_bins,
                                            int hm_elems)
{
    if (!pckt || !src_bins || !hm_bins || (src_elems <= 0) || (hm_elems <= 0) || (pckt->hm_n <= 0))
        return 0;

    for (int i = 0; i < hm_elems; i++)
        hm_bins[i] = pckt->max_llr_bin;

    const int hm_bits = (pckt->hm_n < hm_elems) ? pckt->hm_n : hm_elems;
    for (int i = 0; (i < pckt->info_len) && (i < src_elems) && (i < hm_bits); i++)
        hm_bins[i] = src_bins[i];

    const int parity_bits_in_det = pckt->blk_len - pckt->info_len;
    for (int i = 0; i < parity_bits_in_det; i++) {
        const int src = pckt->info_len + i;
        const int dst = dvc_parity_stream_to_hm_index(pckt, i);
        if ((src >= 0) && (src < src_elems) && (dst >= 0) && (dst < hm_bits))
            hm_bins[dst] = src_bins[src];
    }
                 ...
                 unsigned int ldpc_early_term_6)
{
    ...
    if (ch_llr_mode == 0)
        ...
        sim_pckt->ch_llr_alloc(VENDOR0, (rd_num + 1) / 2, vref);
    else if (ch_llr_mode == 2)
        sim_pckt->ch_llr_alloc(VENDOR1, (rd_num + 1) / 2, vref);
    ...
    printf("[LLR DBG] before ch_llr_gen: ch_sel=%d snr=%f snr_code=%f awgn_sigma=%f sigma2=%f llr_tot_arg=%d llr_frac_arg=%d\n",
           ...
           sim_pckt->awgn_sigma * sim_pckt->awgn_sigma, finite_q_num, kFiniteFracBits);
    sim_pckt->ch_llr_gen(m_llr0, m_llr1, finite_q_num, kFiniteFracBits);
                    ...
                    svOpenArrayHandle llr_tbl_sv)
{
    ...
    if (g_ch_llr_mode == 0)
        ...
        llr_pckt.ch_llr_alloc(VENDOR0, (rd_num + 1) / 2, vref);
    else if (g_ch_llr_mode == 2)
        llr_pckt.ch_llr_alloc(VENDOR1, (rd_num + 1) / 2, vref);
    else
        llr_pckt.ch_llr_alloc(MANUAL, rd_num, vref);
                         ...
                         int h_sc)
{
    ...
    // [DVC IBEX PROOF] Dump raw err_data (det_data_sv) right after svGetArrayPtr
    // to verify the decoder is fed with the exact bits DV passed in.
    // Layout: 16 x 32-bit words per row = 128 hex chars = 512 bits (one circulant).
    // - sd_num<2 : det_data is C-side MSB-first packed. DV SV side prints the SV
    //              packed array via `$displayh(logic[N-1:0])`, which emits bit[0]
    //              as the rightmost hex nibble of the whole integer. So we must:
    //                (a) bit-reverse each 32-bit word (C MSB-first -> LSB-first),
    //                (b) flush 16 words per row with word0 placed at the RIGHTMOST
    //                    position so the full row matches DV's big-integer print.
    // - sd_num>=2: det_data is one int per bit (bin index); natural order, no reversal.
    {
        const int dump_cnt = (sd_num < 2) ? n_words : sim_pckt->blk_len;
        vpi_printf("[DVC IBEX PROOF] ldpc_dec err_data entry: dec_mode=%d sd_num=%d blk_len=%d cnt=%d\n",
                   dec_mode, sd_num, sim_pckt->blk_len, dump_cnt);
        unsigned int words[16];
        int word_cnt = 0;
        int row_idx = 0;
        int start_idx = 0;
        for (int di = 0; di < dump_cnt; di++) {
            unsigned int w = (unsigned int)det_data[di];
            if (sd_num < 2) {
                // bit-reverse 32 bits: flip C's MSB-first word into DV's LSB-first.
            ...
            words[word_cnt++] = w;
            if (word_cnt == 16 || di == dump_cnt - 1) {
                char row[16 * 8 + 1];
                int pos = 0;
                if (sd_num < 2) {
                    // Emit word(word_cnt-1) leftmost, word0 rightmost to match
                    // DV's `$displayh(logic[blk_len-1:0])` big-integer layout.
                    for (int k = word_cnt - 1; k >= 0; k--)
                        pos += snprintf(row + pos, sizeof(row) - pos, "%08x", words[k]);
                } else {
                    // Soft bins: keep natural (index-ascending) order.
                    for (int k = 0; k < word_cnt; k++)
                        pos += snprintf(row + pos, sizeof(row) - pos, "%08x", words[k]);
                }
                row[pos] = '\0';
                vpi_printf("[DVC IBEX PROOF] err_data row%03d [%04d..%04d]: %s\n",
                           row_idx, start_idx, di, row);
                row_idx++;
                start_idx = di + 1;
                word_cnt = 0;
            }
...
void ch_err_inj(svOpenArrayHandle tx_data_sv, svOpenArrayHandle rx_data_sv, int debug)
{
    ...
    vpi_printf("[DVC IBEX DBG] ch_err_inj tx_data first512: ");
    for (int i = 0; (i < 16) && (i < tx_words); i++)
        vpi_printf("%08x", (unsigned int)tx_data[i]);
    vpi_printf("\n");
    ...
    if (use_hm_layout) {
        ...
        for (int i = 0; i < parity_bits_in_det; i++) {
            const int hm_index = dvc_parity_stream_to_hm_index(sim_pckt, i);
            sim_pckt->tx_blk[sim_pckt->info_len + i] =
                (char)dvc_get_bit_from_words_msb32(tx_data, hm_index);
        }
        if (debug >= 1) {
    ...
    if ((debug >= 1) && (sim_pckt->hm_n > 0) && (sim_pckt->hm_m > 0) && (sim_pckt->hm_k > 0) &&
        ...
        if (!cw_full) {
            ...
            if ((sim_pckt->h_matrix.unused_bytes_of_parity > 0) && (parity_bits_in_det < sim_pckt->hm_m)) {
                for (int i = 0; i < parity_bits_in_det; i++) {
                    const int dst = dvc_parity_stream_to_hm_index(sim_pckt, i);
                    if ((dst >= 0) && (dst < sim_pckt->hm_n))
                        cw_full[dst] = (char)(sim_pckt->tx_blk[sim_pckt->info_len + i] & 1);
                }
            } else {
                for (int i = 0; i < parity_bits_in_det; i++) {
...
// Legacy-compatible hard error injection path.
// Behavior matches DVCtrans/src/ldpc_c_model.c::ch_err_inj:
// - TX unpack: MSB-first, sequential first blk_len bits.
// - hard detector: ch_detector(1, &vref0), vref0=0.
// - RX pack: MSB-first, blk_len bits.
extern "C"
void ch_err_inj_legacy(svOpenArrayHandle tx_data_sv,
                       ...
                       int debug)
{
    ...
    if (!sim_pckt || !g_configured) {
        printf("[DVC IBEX ERROR] ch_err_inj_legacy called before ldpc_config.\n");
        return;
    }
    ...
    if ((tx_data == NULL) || (det_data == NULL)) {
        printf("[DVC IBEX ERROR] ch_err_inj_legacy got null SV array pointer.\n");
        return;
    }
                           ...
                           int debug)
{
    ...
    if (sim_pckt->sd_type == MANUAL) {
        ...
        printf("[DVC IBEX WARN] sd_err_inj unsupported sd_type=%d, fallback MANUAL detector\n", (int)sim_pckt->sd_type);
        for (int i = 0; i < sim_pckt->blk_len; i++)
            sim_pckt->det_blk[i] = 0;
        for (int rd_indx = 0; rd_indx < rd_num; rd_indx++) {
            const int split = (sim_pckt->bin_split && (rd_indx < sim_pckt->rd_num))
    ...
    //   [User Data][Valid Parity][tail zero]
    const int bin_elems = svHigh(rx_data_sv, 1) + 1;
    const int use_bin_hm_layout = dvc_fill_sd_bin_data_tail_mask_view(sim_pckt, bin_data, bin_elems);
    ...
    if (debug >= 1) {
        printf("[DVC IBEX] sd_err_inj BIN layout=%s: bin_elems=%d hm_n=%d blk_len=%d pad_bits=%d parity_mask_bits=%d tail_zero_bits=%d max_llr_bin=%d\n",
               use_bin_hm_layout ? "HM_N+pad/max" : "BLK+tail0",
    ...
    if (debug >= 1) {
        char dump_path[256];
        snprintf(dump_path, sizeof(dump_path), "./output/ldpc_dbg_sd_err_inj_io_%04d.txt", g_sd_err_inj_dump_seq++);
        FILE *fp = fopen(dump_path, "w");
        if (fp) {
            const int llr_cnt = (sim_pckt->bin_num > 0 && sim_pckt->bin_num < 128) ? sim_pckt->bin_num : 128;
            const int split_cnt = (rd_num < 128) ? rd_num : 128;
            const int bin_id_cnt = ((rd_num + 1) < 128) ? (rd_num + 1) : 128;
            fprintf(fp,
                    "[SD_ERR_INJ] tx_words=%d bin_elems=%d bin_layout=%s blk_len=%d info=%d hm_k=%d hm_n=%d rd_num=%d tail_zero_bits=%d rd_elems=%d rd_bits_per_plane=%d rd_words_per_plane=%d rd_layout=%s\n",
                    tx_words, bin_elems, use_bin_hm_layout ? "HM_N+pad/max" : "BLK+tail0",
                    sim_pckt->blk_len, sim_pckt->info_len, sim_pckt->hm_k, sim_pckt->hm_n, rd_num, bin_tail_zero_bits,
                    rd_elems, rd_bits_per_plane, rd_words_per_plane,
                    use_rd_hm_layout ? "HM_N+tail0" : "BLK");
            dvc_dump_float_array_fp(fp, "[VREF]", vref, rd_num);
            dvc_dump_words_hex_rows_fp(fp, "[TX_DATA]", tx_data, tx_words);

            int *hm_view_bins = NULL;
            if (sim_pckt->hm_n > 0)
                hm_view_bins = (int *)calloc(sim_pckt->hm_n, sizeof(*hm_view_bins));
            if (hm_view_bins) {
                const int hm_view_ok =
                    dvc_fill_actual_bin_data_hm_view(sim_pckt, bin_data, bin_elems, hm_view_bins, sim_pckt->hm_n);
                const int user_data_start = 0;
                const int user_data_bits = sim_pckt->info_len;
                const int user_padding_start = sim_pckt->info_len;
                const int user_padding_bits =
                    (sim_pckt->hm_k > sim_pckt->info_len) ? (sim_pckt->hm_k - sim_pckt->info_len) : 0;
                const int parity_bits_in_det = sim_pckt->blk_len - sim_pckt->info_len;
                int vp_a_bits = parity_bits_in_det;
                int pmsk_bits = 0;
                int vp_b_bits = 0;
                if ((sim_pckt->h_matrix.extra_bits_of_parity > 0) && (sim_pckt->cir_sz > 0)) {
                    vp_a_bits = sim_pckt->h_matrix.extra_bits_of_parity;
                    if (vp_a_bits > sim_pckt->cir_sz)
                        vp_a_bits = sim_pckt->cir_sz;
                    if (vp_a_bits > parity_bits_in_det)
                        vp_a_bits = parity_bits_in_det;
                    pmsk_bits = sim_pckt->cir_sz - vp_a_bits;
                    if (pmsk_bits < 0)
                        pmsk_bits = 0;
                    vp_b_bits = parity_bits_in_det - vp_a_bits;
                    if (vp_b_bits < 0)
                        vp_b_bits = 0;
                }

                const int vp_a_start = sim_pckt->hm_k;
                const int pmsk_start = vp_a_start + vp_a_bits;
                const int vp_b_start = (pmsk_bits > 0) ? (sim_pckt->hm_k + sim_pckt->cir_sz)
                                                       : (vp_a_start + vp_a_bits);
                if (vp_b_start + vp_b_bits > sim_pckt->hm_n)
                    vp_b_bits = sim_pckt->hm_n - vp_b_start;
                if (vp_b_bits < 0)
                    vp_b_bits = 0;

                fprintf(fp,
                        "[BIN_DATA_ACTUAL_HM_VIEW] source=actual_sv_bin_data one-hex-digit per bin, 32 bins per row, view=%s max_llr_bin=%d\n",
                        hm_view_ok ? "[User Data][User Padding=max][VP_A][PMSK=max][VP_B]" : "unavailable",
                        sim_pckt->max_llr_bin);
                dvc_dump_bins_hex_range_fp(fp, "[BIN_DATA_ACTUAL_HM_VIEW.USER_DATA]", hm_view_bins, user_data_start,
                                           user_data_bits);
                dvc_dump_bins_hex_range_fp(fp, "[BIN_DATA_ACTUAL_HM_VIEW.USER_PADDING_MAX]", hm_view_bins,
                                           user_padding_start, user_padding_bits);
                dvc_dump_bins_hex_range_fp(fp, "[BIN_DATA_ACTUAL_HM_VIEW.VP_A]", hm_view_bins, vp_a_start, vp_a_bits);
                dvc_dump_bins_hex_range_fp(fp, "[BIN_DATA_ACTUAL_HM_VIEW.PMSK_MAX]", hm_view_bins, pmsk_start, pmsk_bits);
                dvc_dump_bins_hex_range_fp(fp, "[BIN_DATA_ACTUAL_HM_VIEW.VP_B]", hm_view_bins, vp_b_start, vp_b_bits);
                free(hm_view_bins);
            } else {
                dvc_dump_bins_hex_rows_fp(fp, "[BIN_DATA] one-hex-digit per bin, 32 bins per row", bin_data,
                                          bin_elems);
            }

            dvc_dump_sd_rd_planes_hex_rows_fp(fp, rd_data, rd_num, rd_bits_per_plane);
            dvc_dump_int_array_fp(fp, "[LLR_TBL]", llr_tbl, llr_cnt);
            dvc_dump_int_array_fp(fp, "[SPLIT_BIN]", split_bin, split_cnt);
            dvc_dump_int_array_fp(fp, "[BIN_ID]", bin_id, bin_id_cnt);
            fclose(fp);
            printf("[DVC IBEX] sd_err_inj IO dump: %s\n", dump_path);
        } else {
            printf("[DVC IBEX WARN] sd_err_inj failed to open IO dump file: %s\n", dump_path);
        }
    }
}
