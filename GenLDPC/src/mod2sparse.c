...
/* WRITE Shift Keys of base matrix */

int mod2sparse_write_skey
( FILE *f,
  mod2sparse *m,
  int g
)
...
void sub_matrix(mod2sparse *A, mod2sparse *subA, int row_start, int row_end, int col_start, int col_end)
{
  mod2sparse *temp = mod2sparse_allocate((row_end - row_start + 1), mod2sparse_cols(A));
  int *rows = chk_alloc((row_end - row_start + 1), sizeof(int));
  int *cols = chk_alloc((col_end - col_start + 1), sizeof(int));
  int i, j;
  int m1, m2, n1, n2;
  ...
  mod2sparse_copyrows(A, temp, rows);
  m1 = mod2sparse_rows(temp);
  n1 = mod2sparse_cols(temp);
  m2 = mod2sparse_rows(subA);
  n2 = mod2sparse_cols(subA);

  mod2sparse_copycols(temp, subA, cols);
