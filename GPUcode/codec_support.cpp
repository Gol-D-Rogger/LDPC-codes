...
void check_nodes::print(void) {
  for (int idx = 0; idx < MAX_ROW_COUNT; idx++) {
    std::cout << "Row " << std::dec << std::setw(2) << std::setfill('0') << idx
              << ": ";
    for (int i = 0; i < 8; i++)
      std::cout << std::uppercase << std::hex << std::setw(16)
                << std::setfill('0') << rows[idx][i] << " ";
    std::cout << "\n";
  }
  std::cout << std::endl;
  return;
...
check_nodes codec_support::config_check_nodes(const h_matrix &h_matrix_ref,
                                              const codeword &vn) {
  check_nodes cn;
  ...
  for (col_idx = 0; col_idx < h_matrix_ref.cols; col_idx++) {
    for (word_idx = 0; word_idx < codeword::WORD_COUNT; word_idx++) {
      ...
      for (uint64_t bit_idx_in_word = 0; bit_idx_in_word < codeword::WORD_SIZE;
           bit_idx_in_word++) {
        ...
        if (word & (UNIT << bit_idx_in_word)) {
          for (row_idx = 0; row_idx < h_matrix_ref.rows; row_idx++) {
            uint64_t rotated_bit_idx =
                (bit_idx + h_matrix_ref.bits -
                 h_matrix_ref.element[row_idx][col_idx]) %
                h_matrix_ref.bits;
            uint64_t rotated_word_idx = rotated_bit_idx / 64;
            uint64_t rotated_bit_idx_in_word = rotated_bit_idx % 64;
            if (h_matrix_ref.occupied[row_idx][col_idx] &&
                (row_idx < (h_matrix_ref.rows - 1)))
              cn.rows[row_idx][rotated_word_idx] ^=
                  (UNIT << rotated_bit_idx_in_word);
          }
        }
      }
    }
...
void codec_support::rotate_left_512(const uint64_t in[8], uint64_t out[8],
                                    unsigned r) {
  r &= 511;
...
void codec_support::mask_range_512(uint64_t out[8], unsigned start,
                                   unsigned len) {
  ...
  unsigned len_remaining =
      (hole_len > bits_in_first_word) ? (hole_len - bits_in_first_word) : 0;
