...
struct ldpc_decoder_input {
  ...
  ldpc_decoder_input();
  ldpc_decoder_input(uint64_t nand_strobes, uint64_t soft_bits,
                     uint64_t iteration_limit, uint64_t post_iteration);
  void clear_cw(void);
};
...
public:
  ...
  struct ldpc_decoder_params {
    ...
    uint16_t likelihood_init_coef_all[8][4] = {
        {0, 7, 6, 5}, {0, 7, 5, 5}, {0, 6, 5, 4}, {0, 5, 5, 4},
        {0, 4, 4, 3}, {0, 3, 3, 2}, {0, 4, 3, 2}, {0, 4, 3, 2},
    };
    uint16_t likelihood_init_fraction[3] = {
        0, 6, 6}; // strobe 5: {0,6,6}, strobe 7: {0,8,4}
...
private:
  void update_vn_planar(uint64_t *vn_flipped, const uint8_t *weight_word,
  ...
  s_likelihood_levels compute_likelihood_levels(
...
struct logger {
  ...
  static void
  log_elapsed_time(const std::chrono::steady_clock::time_point &start_time,
                   const std::chrono::steady_clock::time_point &end_time);

  static void print_accumulated_stats(uint64_t accumulated_cw_count,
