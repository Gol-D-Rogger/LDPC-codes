...
class codec_support {
  public:
    ...
    static void rotate_left_512(const uint64_t in[8], uint64_t out[8], unsigned r);
    static void mask_range_512(uint64_t out[8], unsigned start, unsigned len);
    static check_nodes config_check_nodes(const h_matrix &h_matrix_ref, const codeword &vn);
