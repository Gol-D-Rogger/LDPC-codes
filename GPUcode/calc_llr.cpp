...
double Qinv(double rber) { return inv_norm_cdf(1.0 - rber); }
...
void ch_llr_alloc(int sd_bit, const float *vref_in, int llr_tot_bit,
                  int llr_frac_bit, float hdo_llr, float hd1_llr, float rber,
                  float llr_tbl_return[8], float &awgn_sigma) {
  int sd_num = sd_bit;
  int rd_num = sd_num;
  ...
  int *bin_split = (int *)std::calloc(rd_num, sizeof(*bin_split));
  int *bin_asc_ord = (int *)std::calloc(rd_num + 1, sizeof(*bin_asc_ord));
  ...
  int *sd_asc_ord = (int *)std::calloc(rd_num + 1, sizeof(*sd_asc_ord));
  ...
  float llr_max =
      (std::pow(2.0f, llr_tot_num - 1) - 1.0f) / std::pow(2.0f, llr_frac_num);
  float llr_min =
      (1.0f - std::pow(2.0f, llr_tot_num - 1)) / std::pow(2.0f, llr_frac_num);
  ...
  if (rd_num > 0) {
    ...
    for (int rd_indx = 1; rd_indx < rd_num; rd_indx++) {
      if (vref[rd_indx] > vref_asc_ord[rd_indx - 1]) {
  ...
  if (rd_num == 1) {
    ...
    llr_asc_ord[1] = hdo_llr;
  } else if (rd_num > 1) {
    llr_asc_ord[0] =
        (float)Sat_Quan((double)((-1.0f + vref_asc_ord[0]) / awgn_sigma_sqaure),
                        llr_max, llr_min, llr_tot_num, llr_frac_num);
    for (int i = 1; i < rd_num; i++) {
      llr_asc_ord[i] = (float)Sat_Quan(
          (double)(((vref_asc_ord[i - 1] + vref_asc_ord[i]) * 0.5f) /
                   awgn_sigma_sqaure),
          llr_max, llr_min, llr_tot_num, llr_frac_num);
    }
    llr_asc_ord[rd_num] = (float)Sat_Quan(
        (double)((1.0f + vref_asc_ord[rd_num - 1]) / awgn_sigma_sqaure),
        llr_max, llr_min, llr_tot_num, llr_frac_num);
  }

  // Step 3. Determine soft data from NAND.
  int **nand_read = (int **)std::calloc(rd_num + 1, sizeof(*nand_read));
  for (int i = 0; i <= rd_num; i++)
    nand_read[i] = (int *)std::calloc(rd_num, sizeof(**nand_read));
  ...
  for (int i = 0; i <= rd_num; i++) {
    sd_asc_ord[i] = (rd_num > 0) ? nand_read[i][0] : 0;
  ...
  for (int i = 0; i < bin_num; i++)
    bin_distr[i] = 0;
  for (int i = 0; i <= rd_num; i++) {
    if (sd_asc_ord[i] >= 0 && sd_asc_ord[i] < bin_num)
      bin_distr[sd_asc_ord[i]] = i;
  }
  ...
  if (bin_num >= 8) {
    ...
    for (int i = 0; i < 8; i++)
      llr_tbl_return[i] = (i < bin_num) ? llr_tbl[i] : 0.0f;
  }

  printf("final LLR table=%f %f %f %f %f %f %f %f\n", llr_tbl_return[0],
         llr_tbl_return[1], llr_tbl_return[2], llr_tbl_return[3],
         llr_tbl_return[4], llr_tbl_return[5], llr_tbl_return[6],
         llr_tbl_return[7]);

  std::free(vref);
  std::free(llr_tbl);
  std::free(bin_split);
  std::free(bin_asc_ord);
  std::free(llr_asc_ord);
  std::free(vref_asc_ord);
  std::free(sd_asc_ord);
  std::free(bin_distr);
}
