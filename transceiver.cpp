...
// z --> CH parameter
//############################
void ch_packet::ch_config(int w, int x, int storage_len, ch_model y, float z)
{
    info_len = w;
    blk_len = storage_len;
    ...
    {
        snr = z;
        snr_code = snr - 10 * log10(real_len * 1.0 / info_len);
...
void ch_packet::ch_llr_alloc(enum sd_mode sd_sel, int sd_bit, float *vref_in)
{
    ...
    bin_split = (char *)calloc(bin_num, sizeof(*bin_split));
    bin_asc_ord = (char *)calloc(bin_num, sizeof(*bin_asc_ord));
    vref_asc_ord = (float *)calloc(bin_num, sizeof(*vref_asc_ord));
    llr_asc_ord = (float *)calloc(bin_num, sizeof(*llr_asc_ord));
    sd_asc_ord = (char *)calloc(bin_num, sizeof(*sd_asc_ord));
    bin_distr = (int *)calloc(rd_num, sizeof(*bin_distr));
}
...
void ch_packet::ch_transmit()
{
    ...
    if ((ch_sel == ERR_INJ) || (ch_sel == MAX_ERR))
    {
        if (ch_sel == ERR_INJ)
            ...
            raw_num_err = rand_int(err_num + 1);

        err_vec = (int *)calloc(real_len, sizeof(*err_vec));
        err_pos = (int *)calloc(raw_num_err, sizeof(*err_pos));

        randomBinError(err_vec, err_pos, real_len, raw_num_err);
    }
...
void ch_packet::ch_detector()
{
    ...
    if ((ch_sel == ERR_INJ) & (err_cnt != raw_num_err))
    {
        printf("[SIM]!!!! Error numbers mismatch (%d/%d) !!!!\n", err_cnt, raw_num_err);
    }
    if ((ch_sel == MAX_ERR) & (err_cnt != raw_num_err))
    {
        printf("[SIM]!!!! Error numbers mismatch (%d/%d) !!!!\n", err_cnt, raw_num_err);
    }
#endif
