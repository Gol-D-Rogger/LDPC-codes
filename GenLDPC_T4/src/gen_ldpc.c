...
int gen_ldpc_matrix(mod2sparse *basePCH, mod2sparse *exPCH, int rownum, int colnum, int ex_factor, int pad_bit, int g, WD_vector *row_dt, WD_vector *col_dt, int maximum_girth, int minimum_girth, int** occupied_matrix, int** fade_matrix )
{
    ...
    last_element[0]  =   (4*shift_delta[0]);
    last_element[1]  =   (4*shift_delta[1]);
    last_element[2]  =   (4*shift_delta[2]);
    last_element[3]  =   (4*shift_delta[3]);
    last_element[4]  =   (3*shift_delta[4]);
    last_element[5]  =   0;
    last_element[6]  =   0;
    last_element[7]  =   0;
    last_element[8]  =   0;
    ...
    for (i = 0; i < g; i++)
    {
        ...
        for (int j = colnum - g; j < colnum; j++)
        {
            ...
            if (use_location)
            {
                ...
                e->shift = (g_ind * shift_delta[i])%ex_factor;
        ...
        for(k=0; k<pseudo_weight; k++)
        {
            ...
            for (tmp_girth = tmp_max_girth; tmp_girth >= minimum_girth; tmp_girth -= 2)
            {
                ...
                while (!inserted)
                {
                    ...
                    {
                        e_pre0 = mod2sparse_first_in_row(basePCH, current_row);
                    ...
                    {
                        e_pre0 = mod2sparse_first_in_row(basePCH, current_row);
                        e_pre1 = mod2sparse_first_in_row(basePCH, rownum - 1);
                        mod2sparse_insert(basePCH, current_row, current_col);
                    ...
                    while ((try_cnt < ex_factor) && (cycle_deg_constraint == 1))
                    {
                        if (is_drop_loc==1 && current_col == colnum - rownum)
                        {
                            shift0 = (e_pre0->shift - shift_delta[current_row] + ex_factor)%ex_factor;
                            shift1 = 0;
                        ...
                        {
                            shift0 = (e_pre0->shift - shift_delta[current_row] + ex_factor)%ex_factor;
                            shift1 = (e_pre1->shift - shift_delta[rownum-1] + ex_factor)%ex_factor;
                            insert_sub_matrix_col_mask(exPCH, current_row*ex_factor, current_col*ex_factor,
                    ...
                    int g, int start_girth, int end_girth, float expected_avg_girth)
{
    ...
    while (file_counter < filenum)
    {
        ...
        gen = gen_ldpc_matrix(baseMatrix, exMatrix, b_rows, b_cols,
                         ex_factor, drop_len, g, row_dt, col_dt, start_girth, end_girth, occupied_matrix, fade_matrix);
