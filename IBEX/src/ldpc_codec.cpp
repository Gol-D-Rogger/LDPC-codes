...
void ldpc_packet::ldpc_config(int m, int n, int sc, int st, int wt, char *pchk_file)
{
    ...
    else if (sc == 512)
    {
        ...
        h_matrix.unused_bytes_of_parity = (h_matrix.rows * (h_matrix.bits >> 3)) - h_matrix.bytes_of_parity; // padding bytes
        if (h_matrix.unused_bytes_of_parity != 0)
            h_matrix.extra_bytes_of_parity = (h_matrix.bits >> 3) - h_matrix.unused_bytes_of_parity; // fractional_parity_bytes
        else
            ...
            for (i = 0; i < 5; i++)
            {
                for (m = 0; m < h_matrix.bits; m++)
                {
                    parity[h_matrix.rows - 5 + j][k] ^=
                        (cn.r[i].b[m] & encoder_matrix_first_col[j][i][(h_matrix.bits + m - k) %
                                                                        h_matrix.bits]);
                }
            }
        }
...
ldpc_packet::ldpc_dec_bf_ibex(s_ldpc_decoder_input ldpc_decoder_input, s_ldpc_decoder_parameters ldpc_decoder_parameters, s_h_matrix h_matrix)
{
    ...
    if (ldpc_decoder_input.syndrome_cal_only == 1)
    {
        finished = 1;
    }

    likelihood_levels = f_likelihood_levels(ldpc_decoder_input.nand_strobes, ldpc_decoder_parameters, syndrome_weight, h_matrix.rows);
    ...
    while ((iteration < ldpc_decoder_input.iteration_limit) && (finished == 0) && (give_up == 0))
    {
        for (j = 0; j < h_matrix.cols; j++)
        {
            ...
            {
                for (i = 0; i < 256; i++)
                    prng_256.b[i] = prng_init[i / 16] >> (i % 16) & 1;
                for (i = 0; i < 512; i++)
                    prng_512.b[i] = prng_init[i / 16] >> (i % 16) & 1;
                for (i = 0; i < 512; i++)
            ...
            if (iteration == 0)
                syndrome_weight_delayed = (j <= 3) ? syndrome_weight_r[j] : syndrome_weight_r[4];
            else
                ...
                {   
                    ...
                    if (iteration == 0)
                    {
                        if (ldpc_decoder_input.corrupted_codeword.c[j].b[k].bit_is_error)
                            hard_codeword.errors_at_level_and_weight[ldpc_decoder_input.corrupted_codeword.c[j].b[k].level]
                                                                    [weight]++;
                        else
                            hard_codeword
                    ...
                    vn.c[j].b[k].likelihood = 
                        f_update_vn_post(vn.c[j].b[k].likelihood, w, likelihood_levels.min, likelihood_levels.max,
                                         prng_post_process, prng_post_process2, aggr, likelihood_levels.flip_thr, look);
