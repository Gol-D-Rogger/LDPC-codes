...
s_likelihood_levels ldpc_packet::f_likelihood_levels(int strobes, s_ldpc_decoder_parameters ldpc_decoder_parameters,
  ...
  // ================= 1-bit 专用初始化（v1 框架） =================
  // 在 VN_BITS==1 时，仅作为“初始翻转状态”的配置器：
  // - 数值域固定为 {0,1}
  // - flip_thr 固定为 1（likelihood>=1 即视为翻转）
  // - 所有 level 初始为 0（不利用 soft 做梯度，只做静态偏置）
  if (VN_BITS == 1) {
    likelihood_levels.flip_thr = 1;
    likelihood_levels.weak = 0;
    likelihood_levels.strong = 0;
    likelihood_levels.level[0] = 0;
    likelihood_levels.level[1] = 0;
    likelihood_levels.level[2] = 0;
    likelihood_levels.level[3] = 0;
    likelihood_levels.min = 0;
...
int ldpc_packet::f_update_vn_post(int likelihood, int weight, int min_likelihood, int max_likelihood, bool post_process,
  ...
  // ================= 1-bit 专用更新逻辑：v14 高阈值双向判决 =================
  // 在 VN_BITS==1 时，将 likelihood 视为布尔状态：
  //   0 -> 未翻转，1 -> 已翻转，flip_thr 固定为 1。
  // 使用更保守的静态双阈值：
  //   - 未翻转：weight == 4 时翻转（仅在所有连接校验都报错时才翻）；
  //   - 已翻转：weight == 0 时撤销（所有校验都认为该比特正确时才撤销）。
  if (VN_BITS == 1) {
...
void ldpc_packet::ldpc_dec_bf_ibex(s_ldpc_decoder_input ldpc_decoder_input,
  ...
  bool look;
  int row_unsat_count[13];
  int col_unsat[80];
  int col_unsat_prev[80];
  bool col_unsat_valid = false;
  ...
  for (j = 0; j < h_matrix.cols; j++) {
    for (k = 0; k < h_matrix.bits; k++) {
      ...
      if (VN_BITS == 1) {
        // 1-bit：将 likelihood 固定初始化为未翻转状态（0），
        // 利用 level 记录“soft 可疑度”作为后续记忆的起点：
        // - bit_questionable==0 -> likelihood=0, level=0（起步认为可靠）；
        // - bit_questionable==1 -> likelihood=0, level=1（起步略偏向翻转）。
        vn.c[j].b[k].likelihood = 0;
        vn.c[j].b[k].level = soft_data[0] ? 1 : 0;
      } else {
  ...
  while ((iteration < ldpc_decoder_input.iteration_limit) && (finished == 0) && (give_up == 0)) {
    // 1-bit 模式：每轮迭代开始时按行统计 syndrome 中的 1 的个数，并清零本轮列统计
    if (VN_BITS == 1) {
      for (i = 0; i < h_matrix.rows; i++) {
        int cnt = 0;
        for (k = 0; k < h_matrix.bits; k++) {
    ...
    for (j = 0; j < h_matrix.cols; j++) {
      ...
      if (iteration >= ldpc_decoder_input.post_iteration) {
        ...
        post_trigger2 = 0;
          ...
          // be_aggressive:
          // - VN_BITS==1: 作为“迭代阶段”标记使用（晚期更激进）
          // - 其他位宽：保持原有含义
          if (VN_BITS == 1) {
            be_aggressive = (iteration >= 100);
          } else {
      ...
      for (k = 0; k < h_matrix.bits; k++) {
        ...
        if (!do_not_use_this_bit) {
          ...
          // Adjust weight if aggressive mode（仅 VN_BITS>1 时启用）
          bool aggr = (VN_BITS > 1) && (ldpc_decoder_input.soft_bits > 0) &&
                      (likelihood_levels.min < ldpc_decoder_parameters.likelihood_thr && !flipped_prev);
          int w = weight;

          if (VN_BITS == 1) {
            // v29：1-bit unique + 记忆 + 结构化随机扰动（5.5dB 尝试）
            // level 用于抑制过度翻转：strong_suspect 时 level++，良好时 level--；
            // strong_suspect 定义同 v28。

            short &mem = vn.c[j].b[k].level;

            bool strong_suspect = (u1 > 0) || (u1 == 0 && u2 >= 2 && weight >= 3);
            if (strong_suspect) {
              if (mem < 3)
                mem++;
            } else if (weight == 0 && u1 == 0 && u2 == 0 && mem > 0) {
              mem--;
            }

            bool flipped_new = flipped_prev;

            if (!flipped_prev) {
              // 确定性翻转：记忆高且局部结构明显
              if (mem >= 2 && strong_suspect)
                flipped_new = true;
              // 结构化随机扰动：中等嫌疑度 + 后处理 + pushing + prng_post_process2
              else if (mem == 1 && strong_suspect && (iteration >= ldpc_decoder_input.post_iteration) && pushing &&
                       prng_post_process2)
                flipped_new = true;
            } else {
              if (mem == 0 && !pushing && (weight <= 1) && (u1 == 0) && (u2 == 0))
                flipped_new = false;
            }

            vn.c[j].b[k].flipped = flipped_new;
            vn.c[j].b[k].likelihood = flipped_new ? 1 : 0;
          } else {
            if (aggr && (weight == 0))
              w = weight + 0;
            if (aggr && (weight == 1))
    ...
    // 记录当前轮的列级 weight，用于下一轮结构判据（仅 1-bit 模式）
    if (VN_BITS == 1) {
      for (j = 0; j < h_matrix.cols; j++)
        col_unsat_prev[j] = col_unsat[j];
      col_unsat_valid = true;
    }
