...
static int dvc_env_flag_enabled(const char *name) {
  const char *value = getenv(name);
  return value && *value && strcmp(value, "0") != 0;
}

static FILE *dvc_enc_cn_trace_file() {
  static FILE *fp = NULL;
  static int opened = 0;
  if (opened)
    return fp;
  ...
  fp = fopen("./output/c_code/enc_cn_dump.txt", "w");
  if (!fp)
    fprintf(stderr, "[ENC_CN] failed to open ./output/c_code/enc_cn_dump.txt\n");
  return fp;
}

static void dvc_dump_cn_bits(FILE *fp, const char *tag, s_h_matrix h_matrix, s_check_nodes cn) {
  if (!fp)
    return;
  fprintf(fp, "[ENC_CN] %s rows=%d bits=%d\n", tag, h_matrix.rows, h_matrix.bits);
  for (int row = 0; row < h_matrix.rows; row++) {
    fprintf(fp, "[ENC_CN] row=%02d bits=", row);
    for (int bit = 0; bit < h_matrix.bits; bit++)
      fputc(cn.r[row].b[bit] ? '1' : '0', fp);
    fputc('\n', fp);
  }
  fflush(fp);
}
...
void ldpc_packet::ldpc_ibex_phck(s_h_matrix h_matrix) {
  ...
  for (int i = 0; i < bm_m; i++) {
    for (int j = 0; j < bm_n; j++) {
      if ((h_matrix.extra_bits_of_parity>0) && (h_matrix.element[i][j]>=0)) {
        e = mod2sparse_insert(qc_bm, i, j);
        e->shift = h_matrix.element[i][j];
        for (int k = 0; k < cir_sz; k++) {
          if (((i < bm_m-1) && h_matrix.occupied[i][j])
           || ((i < bm_m-1) && h_matrix.fade[i][j] && !h_matrix.mask[j][k])
           || ((i ==bm_m-1) && h_matrix.mask[j][k])) {
            mod2sparse_insert(qc_hm, i * cir_sz + (k-e->shift+cir_sz), j * cir_sz + k);
      ...
      else if ((h_matrix.extra_bits_of_parity==0) && (h_matrix.occupied[i][j]==1)) {
        e = mod2sparse_insert(qc_bm, i, j);
        e->shift = h_matrix.element[i][j];
        for (int k = 0; k < cir_sz; k++) {
          mod2sparse_insert(qc_hm, i * cir_sz + (k-e->shift+cir_sz), j * cir_sz + k);
...
void ldpc_packet::ldpc_gen_inv() {
  FILE *fp;
  mod2sparse *qc_gz;
  mod2sparse *qc_g, *qc_gi;
  mod2sparse *qc_g_d, *qc_gi_d;
  int *qc_g_cols, *qc_g_rows;

  qc_gz = mod2sparse_allocate(hm_m, (bm_m-tm_sz)*cir_sz);
  qc_g = mod2sparse_allocate((bm_m-tm_sz)*cir_sz, (bm_m-tm_sz)*cir_sz);
  qc_gi = mod2sparse_allocate((bm_m-tm_sz)*cir_sz, (bm_m-tm_sz)*cir_sz);

  qc_g_cols = (int *)calloc((bm_m-tm_sz)*cir_sz, sizeof(*qc_g_cols));
  qc_g_rows = (int *)calloc((bm_m-tm_sz)*cir_sz, sizeof(*qc_g_rows));

  for (int i = 0; i < (bm_m-tm_sz)*cir_sz; i++)
    qc_g_cols[i] = i+(bm_k+tm_sz)*cir_sz;
  for (int i = 0; i < (bm_m-tm_sz)*cir_sz; i++)
    qc_g_rows[i] = i;

  // split column first
  mod2sparse_copycols(qc_hm, qc_gz, qc_g_cols);

  // then split row
  mod2sparse_copyrows(qc_gz, qc_g, qc_g_rows);

  printf("[LDPC] Inverse G matrix generated!\n");

  char MH[50] = "./output/H_matrix.txt";
  char MG[50] = "./output/G_matrix.txt";

  printf("[LDPC] Dump H matrix to file %s\n", MH);
  fp = fopen(MH, "w");
  mod2sparse_print(fp, qc_hm);
  fclose(fp);
  printf("[LDPC] Dump G matrix to file %s\n", MG);
  fp = fopen(MG, "w");
  mod2sparse_print(fp, qc_g);
  fclose(fp);

  // generate inverse G matrix (G=H*F)
  printf("[LDPC] Generating inverse G matrix from H matrix ...\n");

  qc_g_d = mod2dense_allocate((bm_m-tm_sz)*cir_sz, (bm_m-tm_sz)*cir_sz);
  qc_gi_d = mod2dense_allocate((bm_m-tm_sz)*cir_sz, (bm_m-tm_sz)*cir_sz);

  mod2sparse_to_dense(qc_g, qc_g_d);
  mod2dense_invert(qc_g_d, qc_gi_d);
  mod2dense_to_sparse(qc_gi_d, qc_gi);

  printf("[LDPC] Inverse G matrix generated!\n");

  char MGi[50] = "./output/Gi_matrix.txt";
  printf("[LDPC] Dump inverse G matrix to file %s\n", MGi);
  fp = fopen(MGi, "w");
  mod2sparse_print(fp, qc_gi);
  fclose(fp);

  char Gi_ibex[50] = "./output/Gi_ibex.txt";
  printf("[LDPC] Dump inverse G matrix to file %s\n", Gi_ibex);
  fp = fopen(Gi_ibex, "w");
  fprintf(fp, "{\n");
  for (int i = 0; i < 5 ; i++) {
    ...
    for (int j=0; j<5; j++) {
      ...
      for (int k = 0; k<cir_sz; k++) {
        if (k == cir_sz-1)
          if (!mod2sparse_find(qc_gi, i*cir_sz, j*cir_sz+k))
            fprintf(fp, "0");
          else
            fprintf(fp, "1");
        else
  ...
  char Gi_rtl[50];
  printf("[LDPC] Dump inverse G matrix to (rtl) file %s\n", Gi_rtl);
  fp = fopen(Gi_rtl, "w");
  int tmp;
  for (int i=0; i<5; i++) {
    for (int j=0; j<5; j++) {
      fprintf(fp, "assign encoder matrix first col 4k[%d][%d] = \n", i, j);
      fprintf(fp, "%d'h", cir_sz);
      tmp = 0;
      for (int k=cir_sz-1; k>=0; k--) {
        if (k%4 == 0)
          tmp += !mod2sparse_find(qc_gi, i*cir_sz, j*cir_sz+k) ? 0 : 1;
        else if (k%4 == 1)
          tmp += (!mod2sparse_find(qc_gi, i*cir_sz, j*cir_sz+k) ? 0 : 1) << 1;
        else if (k%4 == 2)
          tmp += (!mod2sparse_find(qc_gi, i*cir_sz, j*cir_sz+k) ? 0 : 1) << 2;
        else
          tmp += (!mod2sparse_find(qc_gi, i*cir_sz, j*cir_sz+k) ? 0 : 1) << 3;
  ...
  // free up all internals
  mod2sparse_free(qc_gz);
  mod2sparse_free(qc_g);
  mod2sparse_free(qc_gi);
  mod2dense_free(qc_g_d);
  mod2dense_free(qc_gi_d);
  free(qc_g_cols);
  free(qc_g_rows);
} // ldpc_gen_inv
...
s_192_bits ldpc_packet::f_192_bit_lfsr(s_192_bits data_in) {
  s_192_bits data_out;
  int i;
  bool bit0;
  bit0 = data_in.b[0];
...
s_likelihood_levels ldpc_packet::f_likelihood_levels(int strobes, s_ldpc_decoder_parameters ldpc_decoder_parameters,
  ...
  if (strobes > 0) {
    ...
    if (address >= 64)
      address = 63;
    int coef_index = 0;
    if ((rows+1)/2 == 7) coef_index = 1;
    else if ((rows+1)/2 == 8) coef_index = 2;
    else if ((rows+1)/2 == 9) coef_index = 3;
    else if ((rows+1)/2 == 10) coef_index = 4;
    else if ((rows+1)/2 == 11) coef_index = 5;
    else if ((rows+1)/2 == 12) coef_index = 6;
    else if ((rows+1)/2 == 13) coef_index = 7;
    else if ((rows >= 27) && (rows <= 30)) coef_index = 8;
    else if (rows >= 31)        coef_index = 9;
...
void ldpc_packet::ldpc_config(int m, int n, int sc, int st, int wt, char *pchk_file, char *occupied_file, char *fade_file) {
    ...
    int k;
    int bit;
    int h_matrix_index[M];
    FILE *fp_h, *fp_occupied, *fp_fade;
    fp_h = fopen(pchk_file, "r");
    fp_occupied = fopen(occupied_file, "r");
    fp_fade = fopen(fade_file, "r");
    int occu_flag, fade_flag, elem_flag;
    ...
    h_matrix.bytes_of_userdata = info_len / 8;
    h_matrix.bytes_of_parity = (blk_len - info_len) / 8;
    h_matrix.min_rows = 14;
    h_matrix.max_rows = 45;
    ...
    for (i = 0; i < m; i++) {
      for (j = 0; j < m+K; j++)
      {
        fscanf(fp_h, "%d", &elem_flag);
        fscanf(fp_fade, "%d", &fade_flag);
        fscanf(fp_occupied, "%d", &occu_flag);

        if ((j >= h_matrix.cols - h_matrix.rows) && (j < K)) 
          continue;

        if (j < h_matrix.cols - h_matrix.rows) {
          h_matrix.element[i][j] = elem_flag;
          h_matrix.occupied[i][j] = occu_flag;
          h_matrix.fade[i][j] = fade_flag;
        }
        if (j >= K)
        {
          h_matrix.element[i][h_matrix.cols - h_matrix.rows + j - K] = elem_flag;
          h_matrix.occupied[i][h_matrix.cols - h_matrix.rows + j - K] = occu_flag;
          h_matrix.fade[i][h_matrix.cols - h_matrix.rows + j - K] = fade_flag;
        }
      }
    }
...
void ldpc_packet::ldpc_ibex_parameters(int post_process_en = 1, int syndrome_weight_thr_qc = 0,
  ...
  ldpc_decoder_parameters.likelihood_init_coef_all[8][0] = 0;
  ldpc_decoder_parameters.likelihood_init_coef_all[8][1] = 3;
  ldpc_decoder_parameters.likelihood_init_coef_all[8][2] = 3;
  ldpc_decoder_parameters.likelihood_init_coef_all[8][3] = 2;
  ldpc_decoder_parameters.likelihood_init_coef_all[9][0] = 0;
  ldpc_decoder_parameters.likelihood_init_coef_all[9][1] = 3;
  ldpc_decoder_parameters.likelihood_init_coef_all[9][2] = 2;
  ldpc_decoder_parameters.likelihood_init_coef_all[9][3] = 2;
  ...
  ldpc_decoder_parameters.early_terminate_thr[0][7] = 1650;
  ldpc_decoder_parameters.early_terminate_thr[0][8] = 1800;
  ...
  ldpc_decoder_parameters.early_terminate_thr[1][7] = 3300;
  ldpc_decoder_parameters.early_terminate_thr[1][8] = 3600;
...
#include "ldpc_matrix_inverse.h"
  ...
  int byte_data;
  int index;
  const int trace_cn = dvc_env_flag_enabled("IBEX_ENC_TRACE_CN");
  FILE *enc_cn_fp = trace_cn ? dvc_enc_cn_trace_file() : NULL;
  ...
  if (trace_cn)
    dvc_dump_cn_bits(enc_cn_fp, "after_payload", h_matrix, cn);
  ...
  for (j = h_matrix.cols - h_matrix.rows; j < h_matrix.cols - 5; j++) {
    ...
    if (trace_cn) {
      char tag[80];
      snprintf(tag, sizeof(tag), "after_parity_col_%d", j);
      dvc_dump_cn_bits(enc_cn_fp, tag, h_matrix, cn);
    }
  }

  if (trace_cn)
    dvc_dump_cn_bits(enc_cn_fp, "before_last5_inverse", h_matrix, cn);
  ...
  cn = f_check_nodes(h_matrix, vn);
  if (trace_cn)
    dvc_dump_cn_bits(enc_cn_fp, "final_recomputed", h_matrix, cn);
  ...
  if (ldpc_decoder_parameters.early_terminate_dis == 0) {
    ...
    if (ldpc_decoder_input.nand_strobes == 0) {
      ...
      if (h_matrix.rows <= 12)
        ...
        early_term_thr = ldpc_decoder_parameters.early_terminate_thr[1][7];
      else
        early_term_thr = ldpc_decoder_parameters.early_terminate_thr[1][8];
    }
  ...
  while ((iteration < ldpc_decoder_input.iteration_limit) && (finished == 0) && (give_up == 0)) {
    for (j = 0; j < h_matrix.cols; j++) {
      ...
      if ((iteration == (ldpc_decoder_input.post_iteration + 0)) && (j == 0)) {
        for (i = 0; i < Z; i++)
          prng_192.b[i] = (prng_init[int(i / 16)] >> (i % 16)) & 1;
        for (i = 0; i < 192; i++)
          prng_192.b[i] = (0x1fe0 >> (i % 16)) & 1; // to match verilog
        for (i = 0; i < 256; i++)
          prng_256.b[i] = (prng_init[int(i / 16)] >> (i % 16)) & 1;
        for (i = 0; i < 256; i++)
          prng_256.b[i] = (0x1fe0 >> (i % 16)) & 1; // to match verilog
        for (i = 0; i < 512; i++)
        ...
        prng_192 = f_192_bit_lfsr(prng_192);
