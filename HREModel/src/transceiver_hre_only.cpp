...
// z --> CH parameter
//############################
void ch_packet::ch_config(int w, int x, ch_model y, float z, int u, int v, int hre_en)
{
    info_len = w;
    blk_len = x;
    ch_sel = y;
...
{
    // int err_cnt = 0;
    int hre_like_cnt_tmp = 0; // count tx=1→bin0 and tx=0→bin7
    ...
    for (int i = 0; i < blk_len; i++)
    {
        ...
        if (sd_type != DIRECT)
        {
            if ((tx_blk[i] == 1 && det_blk[i] == 0) || (tx_blk[i] == 0 && det_blk[i] == 7))
            {
                hre_like_cnt_tmp++;
            }
        }
