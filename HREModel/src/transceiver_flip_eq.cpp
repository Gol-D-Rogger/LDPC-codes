...
// u --> HRE bit count
// v --> HRE mode
// hre_en --> HRE LLR processing enable
// tfbc --> Target total FBC (0=disabled)
//############################
void ch_packet::ch_config(int w, int x, ch_model y, float z, int u, int v, int hre_en, int tfbc)
{
    info_len = w;
    blk_len = x;
    ch_sel = y;
    ...
    else if (ch_sel == AWGN || ch_sel == TRUNC_AWGN)
    {
        ...
        else
        {
            ...
            if (target_fbc > 0)
                printf("[CH_TRX] Fixed-FBC mode enabled: target_fbc = %d\n", target_fbc);
        }
    }
    else if (ch_sel == BSC)
    {
...
void ch_packet::ch_pckt_alloc()
{
    ...
    hre_vec = (int*)calloc(blk_len, sizeof(*hre_vec));
    hre_pos = (int*)calloc(hre_num, sizeof(*hre_pos));
#ifdef _ASIC_DUMP
...
void ch_packet::ch_transmit()
{
    ...
    // TRUNC_AWGN/TAWGN：在 x0 上构造“条件 AWGN”帧，使每帧硬错误数恒为 k（不叠加 HRE/pre-flip）
    if (ch_sel == TRUNC_AWGN)
    {
        // Fix-FBC policy:
        //   - target_fbc == 0 (or not set): use theo_FBC = floor(N*rber)
        //   - target_fbc  > 0: use target_fbc directly
        //
        // This is a pure TAWGN construction on x0 (no pre-flip): it samples y from the AWGN
        // conditional distribution given exactly k hard-decision errors.
        int k = (target_fbc > 0) ? target_fbc : (int)(blk_len * rber);
        if (k < 0) k = 0;
        if (k > blk_len) k = blk_len;

        // In TRUNC_AWGN mode, ignore HRE/pre-flip injection.
        for (int i = 0; i < blk_len; i++) hre_vec[i] = 0;
        fixed_hre_cnt = 0;
        hre_like_cnt = 0;
        ...
        for (int i = 0; i < blk_len; i++)
        {
            ...
            if (hard_dec != tx_blk[i])
                err_cnt++;
        }

        raw_err_awgn = err_cnt;
        raw_err_num  = err_cnt;
