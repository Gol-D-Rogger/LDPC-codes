...
// initialization
...
#endif
    sim_pckt->ch_config(dsp_info_len, dsp_blk_len, ch_mode, ch_para, hre_bit, hre_mode, hre_dec, target_fbc);
    ...
    for (sim_cnt=0; (((sim_cnt<max_sim_num)||(max_sim_num==0)) || sim_err<max_err_num); sim_cnt++)
    {
        ...
        if ((sim_cnt!=0) && (sim_cnt%sim_step==0))
        {
            ...
            printf("[SIM] FBC(total,x0->x2) : %f\n", raw_err_tot*1.0/sim_cnt);
            printf("[SIM] FBC(TAWGN,x0->x2) : %f\n", raw_err_awgn_tot*1.0/sim_cnt);
            printf("[SIM] Fixed HRE (flip n): %f\n", fixed_hre_tot*1.0/sim_cnt);
        ...
        // bin 统计：按 x0=tx_blk 参照，仅在 8-bin（bin id 0..7）且非 DIRECT 模式下启用
        if (sim_pckt->sd_type != DIRECT && sim_pckt->bin_num == 8)
        {
            int tx0_bin7 = 0, tx0_bin5 = 0, tx0_bin3 = 0, tx0_bin1 = 0, tx0_self = 0;
            int tx1_bin0 = 0, tx1_bin2 = 0, tx1_bin4 = 0, tx1_bin6 = 0, tx1_self = 0;

            for (int i = 0; i < dsp_blk_len; i++)
            {
                char tx0 = sim_pckt->tx_blk[i];
                int  bin = (int)sim_pckt->det_blk[i];

                if (tx0 == 0)
                {
                    if (bin == 7) tx0_bin7++;
                    else if (bin == 5) tx0_bin5++;
                    else if (bin == 3) tx0_bin3++;
                    else if (bin == 1) tx0_bin1++;
                    else if (bin == 0 || bin == 2 || bin == 4 || bin == 6) tx0_self++;
                }
                else
                {
                    if (bin == 0) tx1_bin0++;
                    else if (bin == 2) tx1_bin2++;
                    else if (bin == 4) tx1_bin4++;
                    else if (bin == 6) tx1_bin6++;
                    else if (bin == 1 || bin == 3 || bin == 5 || bin == 7) tx1_self++;
                }
            }

            tx0_bin7_tot += tx0_bin7;
            tx0_bin5_tot += tx0_bin5;
            tx0_bin3_tot += tx0_bin3;
            tx0_bin1_tot += tx0_bin1;
            tx0_self_tot += tx0_self;
            tx1_bin0_tot += tx1_bin0;
            tx1_bin2_tot += tx1_bin2;
            tx1_bin4_tot += tx1_bin4;
            tx1_bin6_tot += tx1_bin6;
            tx1_self_tot += tx1_self;
        }
        ...
        raw_err_awgn_tot += sim_pckt->raw_err_awgn;
        dec_err_tot += sim_pckt->dec_err_num;
        hre_like_tot += sim_pckt->hre_like_cnt;
        fixed_hre_tot += sim_pckt->fixed_hre_cnt;
            ...
            (sim_pckt->mcrc_err==1) || (sim_pckt->lba_err==1) || (sim_pckt->meta_err==1))
        {
            ...
            printf("[SIM] FBC(total,x0->x2) : %f\n", raw_err_tot*1.0/(sim_cnt+1));
            printf("[SIM] FBC(TAWGN,x0->x2) : %f\n", raw_err_awgn_tot*1.0/(sim_cnt+1));
            printf("[SIM] HRE-like  : %f\n", hre_like_tot*1.0/(sim_cnt+1));
            printf("[SIM] Fixed HRE (flip n): %f\n", fixed_hre_tot*1.0/(sim_cnt+1));
    ...
    printf("[STATISTICS] FBC(total,x0->x2) : %f\n", raw_err_tot*1.0/sim_cnt);
    printf("[STATISTICS] FBC(TAWGN,x0->x2) : %f\n", raw_err_awgn_tot*1.0/sim_cnt);
    printf("[STATISTICS] HRE-like  : %f\n", hre_like_tot*1.0/sim_cnt);
    printf("[STATISTICS] Fixed HRE (flip n): %f\n", fixed_hre_tot*1.0/sim_cnt);
    if (sim_pckt->sd_type != DIRECT && sim_pckt->bin_num == 8)
    {
        printf("[STATISTICS] BINSTAT0: 0->7(HRE) %f | 0->5 %f | 0->3 %f | 0->1 %f | 0->self(0/2/4/6) %f\n",
               tx0_bin7_tot*1.0/sim_cnt, tx0_bin5_tot*1.0/sim_cnt, tx0_bin3_tot*1.0/sim_cnt,
               tx0_bin1_tot*1.0/sim_cnt, tx0_self_tot*1.0/sim_cnt);
        printf("[STATISTICS] BINSTAT1: 1->0(HRE) %f | 1->6 %f | 1->4 %f | 1->2 %f | 1->self(1/3/5/7) %f\n",
               tx1_bin0_tot*1.0/sim_cnt, tx1_bin6_tot*1.0/sim_cnt, tx1_bin4_tot*1.0/sim_cnt,
               tx1_bin2_tot*1.0/sim_cnt, tx1_self_tot*1.0/sim_cnt);
    }
...
void read_config_file()
{
    ...
    // Target total FBC (0=disabled, >0=fixed-FBC mode)
    // Priority: 1) Environment variable TARGET_FBC, 2) Config file, 3) Default 0
    const char *env_target_fbc = getenv("TARGET_FBC");
    if (env_target_fbc) {
        target_fbc = atoi(env_target_fbc);
        printf("[CONFIG] Using TARGET_FBC from environment: %d\n", target_fbc);
        // Skip this line in config file if it exists
        char peek_line[800];
        if (fgets(peek_line, 800, fp)) {
            // Check if this line is a number (target_fbc) or comment/text (fdec_max_itr line)
            int temp_val;
            if (sscanf(peek_line, "%d", &temp_val) == 1 && temp_val >= 0 && temp_val <= 10000) {
                // Looks like target_fbc line, consume it
            } else {
                // Not target_fbc, rewind to re-read as fdec_max_itr
                fseek(fp, -strlen(peek_line), SEEK_CUR);
            }
        }
    } else {
        ...
        if (fgets(peek_line, 800, fp)) {
            int temp_val;
            // Check if this line contains target_fbc (a reasonable number)
            if (sscanf(peek_line, "%d", &temp_val) == 1) {
                // Check if next line is fdec_max_itr (typically 20-50)
                long pos2 = ftell(fp);
                char next_line[800];
                int next_val = -1;
                if (fgets(next_line, 800, fp)) {
                    sscanf(next_line, "%d", &next_val);
                }
                fseek(fp, pos2, SEEK_SET);

                // Heuristic: if temp_val is small (0-200) and next_val is 20-50,
                // temp_val is likely target_fbc
                if (temp_val <= 200 && next_val >= 20 && next_val <= 50) {
                    target_fbc = temp_val;
                } else if (temp_val >= 20 && temp_val <= 50) {
                    // temp_val looks like fdec_max_itr, old config format
                    target_fbc = 0;
                    fseek(fp, pos, SEEK_SET);  // Rewind to re-read as fdec_max_itr
                } else {
                    // Ambiguous, assume it's target_fbc
                    target_fbc = temp_val;
                }
            } else {
                // Not a number, old config format
                target_fbc = 0;
