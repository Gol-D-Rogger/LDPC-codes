...
void dsp_packet::dsp_pckt_alloc()
{
    ...
    wr_lba_blk = (char *)calloc(lba_size, sizeof(*wr_lba_blk));
    rd_meta_blk = (char *)calloc(meta_size, sizeof(*rd_meta_blk));
    rd_lba_blk = (char *)calloc(lba_size, sizeof(*rd_lba_blk));
...
void dsp_packet::rand_eng(unsigned int rseed, char *din_blk, char *dout_blk, int blk_len)
{
    ...
    for (int i = 0; i < 32; i++)
    {
        lfsr[31-i] = rseed % 2;
        rseed = rseed / 2;
        //i++;    // question: why is this increment here?
    }
...
void ch_packet::ecc_encoder()
{
    mcrc_gen();
    ldpc_encoder();
}

void ch_packet::ecc_decoder(enum fc_dec_mode fc_mode)
{
...
void dsp_packet::packet_dump(int pckt_id)
{
    ...
    vec_print(user_blk, info_len, fp);
    fclose(fp);
