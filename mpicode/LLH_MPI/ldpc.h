    ...
    for (j = 0; j < h_matrix.cols; j++) {
      if (h_matrix.bits > 256) {
        if (h_matrix.occupied[i][j])
          printf("%03X ", h_matrix.element[i][j]);
        else
          printf("... ");
      } else {
        if (h_matrix.occupied[i][j])
...
s_likelihood_levels f_likelihood_levels(int strobes, s_ldpc_decoder_parameters ldpc_decoder_parameters,
  ...
  int coef_index = 0;
  likelihood_levels.max = (1 << ldpc_decoder_parameters.VN_BITS) - 1;
  likelihood_levels.min = 1;
  likelihood_levels.flip_thr = (ldpc_decoder_parameters.VN_BITS == 3) ? likelihood_levels.max - 3 : likelihood_levels.max - 7;
  ...
  if (strobes > 0) {
    ...
    delta_total = (ldpc_decoder_parameters.VN_BITS == 8) ? ((delta_sum >> 1) + (delta_sum >> 2)) : ((delta_sum >> 2) + (delta_sum >> 3));
...
int f_update_vn_post(int likelihood, int weight, int min_likelihood, int max_likelihood, bool post_process,
                                  bool post_process2, bool be_aggressive, int flip_threshold) {
  int likelihood_new;
  bool clamp_to_min;
...
s_ldpc_decoder_output f_ldpc_decode(s_ldpc_decoder_input ldpc_decoder_input,
  ...
  while ((iteration < ldpc_decoder_input.iteration_limit) && (finished == 0) && (give_up == 0)) {
        ...
        if (do_not_use_this_bit) {
          ...
          vn.c[j].b[k].likelihood =
              f_update_vn_post(vn.c[j].b[k].likelihood, w, likelihood_levels.min, likelihood_levels.max,
                               prng_post_process, prng_post_process2, aggr, likelihood_levels.flip_thr);
    ...
    finished = (f_check_node_weight(h_matrix, cn) == 0);
    clock_cycles++;
    iteration++;
#ifdef _LDPC_DBG_DUMP
		    if (dbg_sw_fp) {
...
void f_create_include_files()
{
    ...
    bool ldpc_matrix_occupied[M][M][N];
    int ldpc_matrix_fade[M][M][N];
    int ldpc_matrix_first_occupied_or_fade_column[M][N];
}

s_hard_codeword f_generate_random_userdata(int rows, int cols, int bits, int extra_bits)
{
	int j;
	int k;
	int all_zero = 1;
	s_hard_codeword userdata;
	int time_value = (unsigned)time(NULL);
	srand(time_value);

	for (j = 0; j < cols; j++) {
		for (k = 0; k < bits; k++) {
			if ((j < (cols - rows - 1)) || ((j == (cols - rows -1)) && (k < extra_bits)))
				userdata.c[j].b[k] = all_zero ? 0 : ((rand() % 2) == 1);
			else
				userdata.c[j].b[k] = 0;
		}
	}
